...
```powershell
python scripts/manuscript_concept_audit.py concept-tree --source source/shadow
```
...
| 概念名稱 | 第一章如何提出 | 第二章是否有支撐 | 第四章是否有操作 | 第五章是否合理回收 | 判定 | 建議 |
| --- | --- | --- | --- | --- | --- | --- |

建議先執行：

```powershell
python scripts/manuscript_concept_audit.py promise-delivery --source source/shadow
```
